...
stat_loop:
lb $t0, ($t9)		# weź do $t0 bieżący znak
...
stat_print_loop:
lb $t5, ($t0)		# załaduj liczbę wystąpień obecnego znaku, żeby sprawdzić czy w ogóle występuje
beqz $t5, stat_print_next	#jeżeli nie występuje, przejdź dalej
...
stat_print_next:
addiu $t0, $t0, 4	# przejdź do następnego znaku
...
f_m1_n_loop:
lw $t8, ($t9)			# wczytaj do $t8 adres węzła
beq $t8, $s5, f_m1_n_next	# jeżeli null, przejdź do następnego węzła
beqz $t8, find_min1_ascii	# jeśli 0 (nie ma węzła, czyli koniec), przejdź do znajdowania min1 spośród znaków ascii
...
f_m1_n_next:
addiu $t9, $t9, 4		# przesuń głowicę czytającą do przodu
j f_m1_n_loop			# powtórz dla następnego węzła

find_min1_ascii:
la $t9, ascii_stats		# $t9 - głowica czytająca
li $t7, 256			# $t7 - licznik do końca ascii_stats

f_m1_a_loop:
lw $t8, ($t9)			# wczytaj wartość z głowicy czytającej
...
f_m1_a_next:
addiu $t9, $t9, 4		# przesuń głowicę czytającą na następne sąowo
...
beqz $t8, find_min2_ascii	# jeśli 0 (nie ma węzła, czyli koniec), szukaj w ascii
lw $t8, ($t8)			# wczytaj do $t8 wartość węzła
...
beq $t1, $s5, end		# $t1 = null => plik wejściowy pusty => zakończ program
...
m1_is_node:
sw $s5, ($t2)			# zapisz wartość null w miejsce adresu w ascii_table

m2:
beq $s5, $t3, tree_save		# jeżeli min2 = null, to znaczy że jest tylko jeden węzeł, czyli root => zakończono tworzenie drzewa -> przejdź do zapisu drzewa w output_buffer
...
addiu $sp, $sp, -16		# przesuń $sp
j create_node_wrap		# przejdź do tworzenia węzła łączącego min1 i min2
m2_is_node:
sw $s5, ($t5)			# zapisz wartość null w miejsce adresu w ascii_table

create_node_wrap:
add $t0, $t0, $t3		# wylicz sumę poddrzew
...
cnw_loop:
addiu $t9, $t9, 4		# przesuń głowicę do przodu
...
ascii_table_clear:
sw $0, ($t6)			# wyzeruj sąowo
addiu $t6, $t6, 4		# przesuń głowicę
lw $t7, ($t6)			# wczytaj następne sąowo
bnez $t7, ascii_table_clear	# powtórz aż nie wyczyścisz wszystkiego co było zapisane
...
tree_save_loop:
lw $t8, -4($t9)			# wczytaj do $t8 wartość ASCII
bltz $t8, ts_node		# sprawdź czy to node

ts_ascii:		# jeżeli to ASCII, to 1. dopisz do kodu drzewa w output_buffer "1" i kod ASCII 2. zapisz kod znaku w ascii_table, 3. przejdź do prawego potomka ostatniego węzła na stosie (dodając 1 do kodu)
...
ts_node:		# jeżeli to node, to dodaj "0" do kodu drzewa w output_buffer, zapisz się na stosie i przejdź do lewego potomka, dodając "1" do kodu znaku
...
marker_loop:
addiu $t9, $t9, 20		# przesuń głowicę dalej
...
encode_wrap:
lw $t0, bit_head		# załaduj adres głowicy piszącej do $t0
...
read_tree_ascii:
sw $s3, ($sp)			# STWÓRZ NOWY LIŚĆ: zapisz adres rodzica
...
go_right:
lw $s3, -12($s3)		# przejdź głowicę do prawego dziecka
li $s0, 1			# ustaw zero_flag na wartość niezerową
j check				# sprawdź czy to liść
go_left:
lw $s3, -8($s3)			# przejdź głowicę do lewego dziecka
check:
...
write:
sb $t8, ($s2)			# zapisz ASCII pod adresem wskazywanym przez głowicę piszącą
...
WW_next:
sll $s0, $s0, 1		# przesuń dane o 1 w lewo
...
append_bit:
lb $s0, ($a1)		# weź bajt spod podanego adresu
...
read_end:
addiu $sp, $sp, 8	# wróć stos
lw $s0, -4($sp)		# przywróć wartość $s0
lw $ra, ($sp)		# przywróć adres powrotu
jr $ra		# wróć skąd przybyłeś

ReadByte:
# Przeczytaj następne 8 bitów. Korzysta z ReadBit, a więc też posługuje się bit_head i bit_counter.
